      ...
      <AppLayout>
        <div className="relative h-full flex flex-col">
          ...
          <div className="p-6 border-b border-slate-800 bg-slate-900/50">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-slate-400">Visão geral das entregas e motoboys em tempo real</p>
              </div>
              <button
                onClick={() => {
                  loadOrders(true);
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                <div className="text-sm text-slate-400 mb-1">Total de Pedidos</div>
                <div className="text-2xl font-bold text-slate-50">{totalOrders}</div>
              </div>
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                <div className="text-sm text-blue-300 mb-1">Recebidos</div>
                <div className="text-2xl font-bold text-blue-300">{statusCounts.PENDING}</div>
              </div>
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
                <div className="text-sm text-amber-300 mb-1">Em Trânsito</div>
                <div className="text-2xl font-bold text-amber-300">{statusCounts.IN_TRANSIT}</div>
              </div>
              <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                <div className="text-sm text-green-300 mb-1">Entregues</div>
                <div className="text-2xl font-bold text-green-300">{statusCounts.DELIVERED}</div>
              </div>
              <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-4">
                <div className="text-sm text-orange-300 mb-1">Motoboys Ativos</div>
                <div className="text-2xl font-bold text-orange-300">{availableMotoboys}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 relative bg-slate-900 min-h-[500px]">
            ...
            {/* Legenda do Mapa */}
            <div className="absolute bottom-4 left-4 bg-slate-900/90 backdrop-blur-sm border border-slate-800 rounded-lg p-4 z-[1000] shadow-xl">
              <div className="text-sm font-semibold text-slate-50 mb-2">Legenda</div>
            ...
            <button
              ...
              className="fixed bottom-6 right-6 w-16 h-16 bg-orange-500 hover:bg-orange-600 text-white rounded-full shadow-lg shadow-orange-500/50 flex items-center justify-center text-2xl z-[1000] transition-all hover:scale-110 hover:shadow-xl"
          ...
          {/* Painel de Status */}
          <aside
            className={`fixed right-0 top-0 h-full w-96 bg-slate-900 border-l border-slate-800 transform transition-transform duration-300 z-20 shadow-xl ${
            ...
            <div className="h-full flex flex-col">
              <div className="p-6 border-b border-slate-800 bg-slate-800/50">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-slate-50">Status de Entregas</h3>
                  <button
                  ...
                  <div className="space-y-3 animate-fade-in">
                    {filteredOrders.map((order) => (
                      <Link
                        ...
                        className="block p-4 bg-slate-800 border border-slate-700 rounded-lg hover:border-orange-500/50 transition-all hover:shadow-lg hover:shadow-orange-500/10 cursor-pointer group"
                      >
